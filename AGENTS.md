...
- **Fixed Navbar Overlap Fix** (`assets/css/custom.css`, `assets/js/custom.js`): 고정 네비게이션 바가 페이지 콘텐츠나 앵커 링크 목표를 가리는 문제 해결. CSS에 `scroll-padding-top` 추가(모바일 70px, 데스크탑 90px)하여 앵커 링크 클릭 시 헤딩이 네비게이션 바 아래에 정확히 위치하도록 수정. JS에 `adjustScrollPadding()` 함수 추가하여 실제 네비게이션 바 높이 + Google Translate 배너 높이를 동적으로 측정, `scroll-padding-top`을 자동 조정. `ResizeObserver`로 네비게이션 바 크기 변화 감지 및 window resize 이벤트에도 재적용.